...
class Framebuffer(val device: Device, val id: Int, val textures: Map<FramebufferAttachment, Image<*, Texture2d>>) {
    ...
    val width = if (id == 0) Client.window.width else textures.values.map(Image<*, Texture2d>::width).max() ?: 0

    val height = if (id == 0) Client.window.height else textures.values.map(Image<*, Texture2d>::height).max() ?: 0
